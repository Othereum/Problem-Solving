...
int solution(string skill, vector<string> skill_trees)
{
    ...
    {
        string learned_pre_skills;
        
        for (char s : skill_tree)
        {
            if (skill.find(s) != string::npos)
            {
                learned_pre_skills += s;
            }
        }
        bool possible_skill_tree = true;
        for (int i = 0; i < learned_pre_skills.size(); ++i)
        {
            if (skill[i] != learned_pre_skills[i])
            {
                possible_skill_tree = false;
                break;
            }
        }
        answer += possible_skill_tree;
    }
